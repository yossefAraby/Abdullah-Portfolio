...
var main = (function($) { var _ = {
	...
	keys: {
		...
		// Right Arrow: Next (for RTL: previous).
			39: function() {
				_.previous();
			},

		// Left Arrow: Previous (for RTL: next).
			37: function() {
				_.next();
			}
			...
			// Touch gestures.
				_.$viewer
					...
					.on('touchmove', function(event) {
						...
						// Swipe left (for RTL: next).
							if ( (diffY < boundary && diffY > (-1 * boundary)) && (diffX > delta) )
								_.next();

						// Swipe right (for RTL: previous).
	...
	initViewer: function() {
		...
		// Create slides from thumbnails.
			_.$thumbnails.children().not('.category-header')
				.each(function() {

					var	$this = $(this),
						$thumbnail = $this.children('.thumbnail'),
						s,
						isVideo = $thumbnail.data('video') === true || _.isVideo($thumbnail.attr('href'));
						...
						if (_.settings.preload) {
							...
							if (isVideo) {
								...
								// Create video element.
									s.$video = $('<video src="' + s.url + '" controls playsinline webkit-playsinline style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:contain;background:#000;"></video>');
									s.loaded = true;
		...
		// Deactivate old slide (if there is one).
			if (oldSlide) {
				...
				// Pause video if switching away.
					if (oldSlide.isVideo && oldSlide.$video) {
						oldSlide.$video[0].pause();
					}
			...
			// Slide.
				var f = function() {
							...
							window.setTimeout(function() {
								...
								if (newSlide.isVideo) {
									...
									// Create and load video.
										newSlide.$video = $('<video src="' + newSlide.url + '" controls playsinline webkit-playsinline style="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:contain;background:#000;"></video>');
										newSlide.$video.on('canplay', function() {

											// Append video to image container.
												newSlide.$video.appendTo(newSlide.$slideImage);
					...
					// Otherwise ...
							...
							// If video, append and play.
								if (newSlide.isVideo && newSlide.$video) {
									if (!newSlide.$video.parent().length) {
										newSlide.$video.appendTo(newSlide.$slideImage);
									}
									newSlide.$video[0].play().catch(function() {});
								}
